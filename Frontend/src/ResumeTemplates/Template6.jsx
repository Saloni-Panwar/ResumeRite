  ...
  return (
    ...
    >
      ...
      <Box marginTop="2rem">
        ...
        {workExperiences.jobTitle.map((title, index) => (
          <Box key={index} marginBottom="1rem">
            ...
            <Typography color="#666">{workExperiences.description[index]}</Typography>
          </Box>
        ))}
      </Box>
      ...
      <Box marginTop="2rem">
        ...
        {/* Display the Education section based on the available education data */}
        {(education.educationType === "Post Graduate" ||
          education.educationType === "Graduate" ||
          education.educationType === "Under Graduate") && (
          <>
            <Box marginBottom="1rem">
              <Typography color="#333" fontWeight="bold">
                {education.degree} at {education.college} ({education.startYear} - {education.endYear})
              </Typography>
              <Typography color={secondaryColor}>{education.university}</Typography>
              <Typography color="#666">{education.description}</Typography>
            </Box>
            {education.graduation && (
              <Box marginBottom="1rem">
                <Typography color="#333" fontWeight="bold">
                  Graduation: {education.graduation} ({education.startYear} - {education.endYear})
                </Typography>
              </Box>
            )}
          </>
        ...
        {education.educationType === "Higher Secondary-12th" && (
          <Box marginBottom="1rem">
            <Typography color="#333" fontWeight="bold">
              {education.school} ({education.yearOfCompletion})
            </Typography>
            <Typography color={secondaryColor}>{education.stream}</Typography>
          </Box>
        )}
