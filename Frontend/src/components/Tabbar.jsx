...
// export default VerticalTabs;
import React, { useState } from "react";
import { Tabs, Tab, Box, useTheme, useMediaQuery, Icon } from "@mui/material";
...
const VerticalTabs = ({ setOnFormSubmit }) => {
  const [activeTab, setActiveTab] = useState(0);
  const [tabFilled, setTabFilled] = useState([false, false, false, false]);
  const theme = useTheme();
  const lightGray = theme.palette.neutral.light;
  const isMobileScreen = useMediaQuery("(max-width:600px)");

  const handleNext = () => {
    // Mark the current tab as filled and move to the next tab
    setTabFilled((prevTabFilled) => {
      const updatedTabs = [...prevTabFilled];
      updatedTabs[activeTab] = true;
      return updatedTabs;
    });
    setActiveTab((prevTab) => Math.min(prevTab + 1, 3));
  };
      ...
      {tabFilled[index] && (
        <Icon
          sx={{
            fontSize: "25px",
            marginLeft: "8px",
            color: "#32CD32", // Green color for the check mark
          }}
        >
          <CheckCircleIcon />
        </Icon>
    ...
    >
      ...
      >
        <Tabs
          ...
          value={activeTab}
        >
          <Tab label={renderTabLabel("Personal Info", 0)} sx={getTabStyle(0)} />
          <Tab label={renderTabLabel("Education", 1)} sx={getTabStyle(1)} />
          <Tab label={renderTabLabel("Work Experience", 2)} sx={getTabStyle(2)} />
          <Tab label={renderTabLabel("Skills", 3)} sx={getTabStyle(3)} />
        </Tabs>
      </Box>
